...
test('time-query: 非 Server 来源 / 非法值不缓存', () => {
  const { bot } = makeEnv()
  bot.emit('chat', 'steve', 'The time is 99999')
  bot.emit('chat', 'Server', 'The time is abc')
  bot.emit('chat', 'Server', '您没有权限执行此命令')
  assert.equal(bot.time.dayTime, undefined, '非法来源/值不缓存')
})

test('time-query: 定时发起查询命令', async () => {
  const { bot } = makeEnv()
  // 首查 1s 后（QUERY_INTERVAL 30s 太长不测）——手动验证 chat 调用存在
  await new Promise((r) => setTimeout(r, 1500))
  assert.ok(bot.chatCalls.length >= 1, '上线后应发起 /time query')
  assert.equal(bot.chatCalls[0], '/time query daytime')
})

test('time-query: 查询缓存驱动 environmentLine（集成）', () => {
  const { bot } = makeEnv()
  bot.emit('chat', 'Server', 'The time is 11767') // 实际 17:46 的 dayTime
