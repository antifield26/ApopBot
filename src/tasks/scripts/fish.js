  ...
  script: {
    steps: [
      { ctrl: 'loop', max: 'infinite', body: [
        ...
        // 背包满（占用槽位 ≥34 视为满，留手持/盔甲位——按槽位数而非物品种类数
        // 判定——observe_inventory 的 slotsUsed 字段）
        ...
        // count 用 {name, field} 形态：fish 原语对抛竿超时/上钩失败返回
        // {caught:false} 且 ok:true（业务性无事可做契约）——字符串形态在 ok:true
        // 时无条件 +1，会把超时也计入 caught（遥测虚高）
