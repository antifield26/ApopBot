// AFK 任务脚本（v1.0.0 C6）：周期性微小视角转动，防 Paper afk-kick-timeout 踢出。
// 语义与原 AfkTask 逐条对应：intervalMinutes ≥1 校验（校验在 task-schemas）、
  ...
  script: {
    steps: [
      { ctrl: 'loop', max: 'infinite', body: [
        ...
        // 这里由原语内部兜底：bot.look 抛错转 {ok:false} 时脚本失败，与原任务
        // "失败仅 warn"语义有差异——但实体缺失属异常态，fail 可被调度重试）
