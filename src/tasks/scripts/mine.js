// 挖矿任务脚本（v1.0.0 C7）：collectblock + pathfinder 在限定区域内挖掘指定方块。
// 语义与原 MineTask 逐条对应：
...
// - 默认 radius 32 / maxBlocks 64（与原任务一致）
    ...
    steps: [
      { ctrl: 'loop', max: 'infinite', body: [
        ...
        { ctrl: 'if', cond: { type: 'result', ref: 'targets', field: 'candidates.length', equals: 0 }, then: [
          // 无目标：stopWhenDone → 完成；否则 5min no-target 内部等待（F8/F3）
          ...
          // 背包满（无箱子可存）→ 5min 等待清空（F2）；其他失败 → 30s 重试
