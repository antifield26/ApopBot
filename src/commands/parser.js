...
// 未闭合引号返回 error（否则消息尾部被静默吞掉）；\" 转义可输入字面双引号。
// 纯函数便于单测。
...
export function parseCommand (line) {
  ...
  let jsonEscaped = false // JSON 字符串值内的转义（\"）——被转义的引号不翻转 inJsonStr
  ...
  for (const ch of trimmed.slice(1)) {
    if (escaped) {
      ...
      // braceDepth 只对 JSON 顶层计数——JSON 字符串值里的 { } 不计入（'{"a": "x{"}'
      // 的 { 若计入会使 depth=2），后续空格不再分割 → 整串合并为一个 token →
      ...
      if (!inQuotes) {
        // JSON 字符串值内先处理转义：`\"` 若按普通引号处理会使 inJsonStr 连续
        // 误翻转两次 → 字符串结束引号后 } 不计 depth → braceDepth 残留 → 后续
        // 参数被并入 JSON token（合法 JSON 参数解析失败，如 {"desc":"say \"hi\""}）
        if (jsonEscaped) {
          jsonEscaped = false
