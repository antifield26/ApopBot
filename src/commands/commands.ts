...
import { findSurfaceBlocks, createMovement, REASON_TEXT } from '../core/movement.ts'
import { validateTaskOptions, validateCron } from '../core/task-schemas.ts'
import { hasExclusiveActive, getExclusiveOwner } from '../core/arbiter.ts'
...
export function registerBuiltinCommands (registry, _ctx) {
  ...
  registry.register({
    ...
    handler: async (c, args) => {
      ...
      const player = Object.values(c.bot.players as Record<string, any>).find(p => p.username.toLowerCase() === lower)
  ...
  registry.register({
    ...
    handler: async (c, args, sender) => {
      const { setPlace, removePlace, listPlaces } = await import('../core/discovery.ts')
