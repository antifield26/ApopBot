import { createMovements } from '../core/movement.ts'
 ...
 */
export async function loadMineflayerPlugins (bot, cfg, logger, deps: Record<string, any> = {}) {
  ...
  // 包装器：注入时执行插件本体 + 记录句柄
  const wrap = (plugin, name, getHandle = null) => {
    bot.loadPlugin((b, o) => {
      plugin(b, o)
  ...
  if (enabled.follow === true) {
    const { followPlugin } = await imp('follow', () => import('./follow.ts'))()
