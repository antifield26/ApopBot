...
class HttpError extends Error {status
...
class CloudProvider {
   ...
   */
  async chat (messages, { tools = [], system = '', signal }: Record<string, any> = {}) {
    if (!this.apiKey) {
    ...
    if (tools.length > 0) {
      body.tools = tools.map(({ name, description, parameters } = {} as Record<string, any>) => ({
        name,
        description,
