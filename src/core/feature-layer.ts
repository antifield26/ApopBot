...
import { createCommandRegistry } from '../commands/commands.ts'
import { createL2 } from '../l2/index.ts'
import { sendChat } from './chat.ts'
import { createNotifier } from './notify.ts'
import * as discovery from './discovery.ts'
import { installChatListener } from './fl-chat.ts'
import { installDeathHandling } from './fl-death.ts'
import { installPlayerTracking } from './fl-players.ts'
import { installMemoryInvalidation, installWorldSensing } from './fl-world.ts'
import { installGuardResponse } from './fl-guard.ts'
import { installTimeQuery } from './time-query.ts'
import { bindIdleWatcher } from './idle-watcher.ts'

// 测试钩子 re-export（动态 import 路径不变——测试零改动）
export { _resetGreetState } from './fl-players.ts'
export { _resetIdleWatcher } from './idle-watcher.ts'

export function createFeatureLayerManager (ctx, logger, deps: Record<string, any> = {}) {
