...
import { createMovement, REASON_TEXT } from '../movement.ts'
import { useEntityOn } from '../entity-actions.ts'
import { SEED_BY_CROP } from '../crops.ts'
import { interruptibleSleep } from './common.ts'
    ...
    handler: async (c, { filter, foodName, count = 2, useCooldownMs = 3000, minFeedIntervalMs = 0 }, runtime) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (dist > 4) {
          ...
          if (!move.ok) {
            return { fed, targetGone: false, targetName: target.name, reason: `无法接近: ${REASON_TEXT[move.reason] ?? (move as Record<string, any>).err?.message}` }
          }
        }
