...
import { exploreStep, notifyValuableFound } from '../explore.ts'
...
export function registerMove (register, _ctx) {
  ...
  register('explore_step', {
    ...
    handler: async (c, { maxDistance, direction }, runtime) => {
      ...
      return { from: [r.from.x, r.from.y, r.from.z], to: [r.to.x, r.to.y, r.to.z], found: r.found.map(f => ({ name: f.name, x: f.x, y: f.y, z: f.z })), hostile: (r.entities as Record<string, any>).hostile ?? [] }
    }
  })
