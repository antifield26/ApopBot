...
import { attackEntity } from '../entity-actions.ts'
import { isArea } from '../../tasks/util.ts'
import { ACTION_COOLDOWN_MS, checkActionCooldown } from './common.ts'
    ...
    handler: async (c, { filter, maxHits, maxDistance, area }, runtime) => {
      ...
      for (let i = 0; i < (maxHits ?? 5); i++) {
        ...
        if (dist > ATTACK_RANGE) {
          ...
          return { hits, targetGone: false, targetName: target.name, reason: `无法接近: ${REASON_TEXT[r.reason] ?? (r as Record<string, any>).err?.message}` }
        }
