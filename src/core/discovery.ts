...
/** 锚点记录（探索记忆坐标）。 */
interface Anchor { x: number; y: number; z: number; ts: number; dimension?: string|null }
/** 命名地点（家/矿场/基地）。 */
interface Place { name: string; x: number; y: number; z: number; dimension?: string|null; ts: number }
/** 危险区域（hostile 实体出没坐标）。 */
interface DangerZone { x: number; y: number; z: number; threatLevel: number; hostileNames: string[]; dimension?: string|null; ts: number }

const state: {
  anchors: Anchor[]
  resources: Record<string, Anchor[]>
  places: Place[]
  dangerZones: DangerZone[]
  store: { setMemory?: (..._args: any[]) => void }|null
} = {
...
export function importSnapshot (memory) {
  ...
  if (memory.resources && typeof memory.resources === 'object' && !Array.isArray(memory.resources)) {
    ...
    for (const [name, list] of Object.entries(res)) {
      for (const r of list as any[]) entries.push({ name, ...r })
    }
 ...
 */
export function recordDangerZone (pos, { hostileNames = [], threatLevel }: Record<string, any> = {}, dimension = null) {
  ...
  const names: string[] = [...new Set<string>((hostileNames as string[]).map(String).filter(Boolean))].slice(0, 8)
  ...
  if (existing) {
    ...
    existing.hostileNames = [...new Set<string>([...existing.hostileNames, ...names])].slice(0, 8)
