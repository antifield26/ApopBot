...
import { isOp } from '../commands/permissions.ts'
import { hasExclusiveActive, getExclusiveOwner } from './arbiter.ts'
import { createPrimitiveRegistry } from './primitives/index.ts'
 ...
 */
export function createActionExecutor (ctx, deps: Record<string, any> = {}) {
   ...
   */
  async function executeBatch (actions, opts: Record<string, any> = {}) {
  ...
  async function executeOne (op, args, opts: Record<string, any> = {}) {
...
export function validateParams (schema, params) {
  ...
  for (const [k, def] of Object.entries((schema.properties ?? {}) as Record<string, any>)) {
