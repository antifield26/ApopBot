...
import { loadConfig, validateConfig, assertLogDirWritable } from './core/config.ts'
import { createLogger } from './core/logger.ts'
import { ConnectionManager } from './core/connection.ts'
import { createFeatureLayerManager } from './core/feature-layer.ts'
import { createReloadHandler } from './core/reload.ts'
import { createL2 } from './l2/index.ts'
import { setupSignals } from './core/signals.ts'
import { createStatusServer } from './core/http-status.ts'
import { createStateStore } from './core/state.ts'
import { createNotifier } from './core/notify.ts'
import { registerChatLogger } from './core/chat.ts'
import * as discovery from './core/discovery.ts'
...
const statusServer = createStatusServer(() => ctx.cfg, logger, () => ({
  ...
  rebuildFails: (ctx as Record<string, any>)._rebuildFails ?? 0, // 功能层重建失败计数（重建失败后 ctx.bot 置空——观测通道）
